...
const Search = () => {
    ...
    //Get the search query for searching using URLsearchparams(location.query)
    ...
    const getSearch = async () => {
        try {
            ...
            //run firebase query based on result of search query
            //1.first query check in products doc in productname feild for greaterthan or lessthan from the search query 
            const q = query(collection(database, "Products"),
                where("productname", "<=", searchquery + "\uf8ff"),
                where("productname", ">=", searchquery),
            );
            //2.check for the category based on search query
